...
const App: React.FC = () => {
  ...
  const handleUpdateConfig = useCallback((updates: Partial<SnowflakeConfig>, commitTo3D: boolean = false) => {
    ...
    setConfig(prev => {
      ...
      // Auto-switch slot mode when enabled layer count changes
      if ('layers' in updates) {
        const newLayers  = (updates.layers as LayerConfig[]) ?? config.layers;
        const prevCount  = config.layers.filter(l => l.enabled).length;
        const nextCount  = newLayers.filter((l: LayerConfig) => l.enabled).length;

        if (nextCount !== prevCount && nextCount >= 2) {
          // Recalculate optimal slots for the new plane count
          updates = {
            ...updates,
            layers: calculateOptimalSlots(newLayers),
          };
          console.log(`🔄 Auto-switched to ${nextCount}-plane slot mode`);
        }
      }
        ...
        if (layerMerged) {
          ...
          // the visual quality high (sharp caps, smooth walls).
           ...
           if (config.slotEnabled || viewMode === '3d') {
             // Minimal weld only — no triangle removal or normal recompute
