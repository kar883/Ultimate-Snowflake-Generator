...
const createSlotGeometries = (
  ...
  // ── Layer 3D rotation ─────────────────────────────────────────────────────
  // The geometry has ALREADY been rotated by layer.rotation3D.x before
  // applySlotCuts is called (see generateMesh: layerMerged.rotateX(...)).
  // Blades therefore operate in WORLD space on the already-tilted mesh.
  //
  // A flat plane (rx=0°) has its face normal along Z.  The blade extent must
  // align with the face normal so it punches straight through the material.
  //
  // BoxGeometry starts with Y = extent axis.  After rotateX(θ), Y maps to:
  //   (0, cos θ, sin θ)
  // We need this to equal the plane normal (0, -sin rx, cos rx), so:
  //   θ = 90° + rx
  //
  // For layer 0 (rx=0°):   θ = 90°  → punches along +Z ✓
  // For layer 1 (rx=120°): θ = 210° → punches along tilted normal ✓
  // For layer 2 (rx=240°): θ = 330° → punches along tilted normal ✓
  const layerRx    = layer.rotation3D?.x ?? 0;
  const bladeAngleX = 90 + layerRx;   // degrees

  // ── createBlade ────────────────────────────────────────────────────────────
  // Creates a slot-cut box aligned to the ALREADY-ROTATED plane's normal.
  //   length    = slot length along the arm
  //   xOffset   = where along local +X (arm dir) the blade starts
  //   thickness = slot opening width (cutThickness)
  //   extent    = FULL_PUNCH — large enough to pass through the whole mesh
  //   angleX    = ignored; bladeAngleX (above) is always used
  //   angleZ    = rotateZ to aim blade along the correct arm direction
  const createBlade = (
    length:    number,
    xOffset:   number,
    thickness: number,
    extent:    number,
    _angleX:   number,  // kept for call-site compat; actual angle = bladeAngleX
    angleZ:    number
  ): THREE_ACTUAL.BufferGeometry => {
    const overlap  = 4.0;
    ...
    geo.rotateX(bladeAngleX * Math.PI / 180);
    geo.rotateZ(angleZ * Math.PI / 180);
    return geo;
  };

  const createVerticalBlade = (length: number, xOffset: number) =>
    createBlade(length, xOffset, cutThickness, FULL_PUNCH, 0, -rotationOffset);
  ...
  //
  //   Use drawLength to match 2D preview exactly
  if (numPlanes === 2) {
    const angle = rotationOffset;
    const angleRad = angle * Math.PI / 180;
    const x2 = Math.cos(angleRad) * drawLength;
    const y2 = Math.sin(angleRad) * drawLength;

    if (layer.slotType === 'half-back') {
      // Slot from center outward (matches 2D preview)
      slots.push(createBlade(drawLength, 0, cutThickness, FULL_PUNCH, 90, -angle));
    } else if (layer.slotType === 'half-front') {
      // Slot from outer edge inward (matches 2D preview)
      slots.push(createBlade(drawLength, -drawLength, cutThickness, FULL_PUNCH, 90, -angle));
    } else {
      // Custom / fallback: full slot centred on origin
      slots.push(createBlade(drawLength, -drawLength / 2, cutThickness, FULL_PUNCH, 90, -angle));
    }
    return slots;
  }
  ...
  if (numPlanes === 3) {
    ...
    if (layerIndex === 0) {
      // Base slot: only ONE slot from center outward (matches 2D preview)
      slots.push(createBlade(drawLength, 0, cutThickness, FULL_PUNCH, 90, -angle));

    } else if (layerIndex === 1) {
      // Cross main slot: from center outward (matches 2D preview)
      slots.push(createBlade(drawLength, 0, cutThickness, FULL_PUNCH, 90, -angle));
      // Tip-in slot: from 75% position outward (matches 2D preview)
      const xStart = Math.cos(angleRad + Math.PI) * (drawLength * 0.75);
      const yStart = Math.sin(angleRad + Math.PI) * (drawLength * 0.75);
      slots.push(createBlade(drawLength * 0.25, xStart, cutThickness, FULL_PUNCH, 90, -(angle + 180)));

    } else if (layerIndex === 2) {
      // Tilt main slot: from center outward (matches 2D preview)
      slots.push(createBlade(drawLength, 0, cutThickness, FULL_PUNCH, 90, -angle));
      // Extension slot: from 75% position outward (matches 2D preview)
      const x1 = Math.cos(angleRad + Math.PI) * (drawLength * 0.75);
      const y1 = Math.sin(angleRad + Math.PI) * (drawLength * 0.75);
      slots.push(createBlade(drawLength * 0.25, x1, cutThickness, FULL_PUNCH, 90, -(angle + 60)));
    }
