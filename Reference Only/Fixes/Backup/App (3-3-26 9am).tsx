...
import { makeCacheKey, getOrCreateSlotGeometries, clearSlotCache } from './slotGeometryCache';
...
const createSlotGeometries = (
  ...
  baseSlotWidth: number,         // kept in signature for API compat
  ...
  // ── Slot dimensions ───────────────────────────────────────────────────────
  // materialThickness drives the opening so the mating plane can slide in.
  // cutDepth is the blade height (extent) — enough to punch through the mesh.
  const materialThickness = extrusionDepth + (bevelEnabled ? bevelAmount * 2 : 0);
  const cutThickness = materialThickness + 0.2 + adjWidth;
  const cutDepth     = materialThickness + 5.0;

  console.log(`🔧 Slot geometry for layer ${layer.id} (${layer.slotType}):`, {
    slotLength, numPlanes, cutThickness, cutDepth, rotationOffset,
  });

  // ── createBlade ────────────────────────────────────────────────────────────
  // Blades are built in LOCAL (pre-rotation) space.
  // The caller (applySlotCuts) is responsible for applying layer.rotation3D
  // to each blade clone before the CSG subtraction.
  //
  // angleX: rotateX tilts the blade extent (height) into the cut direction.
  //   90°  → blade punches straight through a flat (XY) plane along Z.
  //   120° → used for 3-plane mode to match 120° spacing.
  //   240° → complementary 3-plane angle.
  // angleZ: rotates the blade in the XY plane to aim along the snowflake arm.
  const createBlade = (
    length:    number,
    xOffset:   number,
    thickness: number,
  ...
  const createVerticalBlade = (length: number, xOffset: number) =>
    createBlade(length, xOffset, cutThickness, cutDepth, 90, -rotationOffset);

  // ── 2-plane assembly ──────────────────────────────────────────────────────
  // Single vertical slot centred on origin, full slotLength.
  // One plane gets the top half, the other the bottom half (handled by
  // rotation3D.x = 0° vs 90° applied externally in applySlotCuts).
  if (numPlanes === 2) {
  ...
  //
  // Planes interlock at 120° intervals.  Each plane needs two cuts so the
  // other two planes can slide in.  Blade angles 120° and 240° (vs. the flat
  // plane normal at 90°) create cuts that align with the 120° rotated mates.
  //
  // Layer 0 (third-back,   rotation3D.x = 0°):
  //   Two cuts at 120° and 240° from centre — accepts layer 1 & 2 sliding in.
  //
  // Layer 1 (third-middle, rotation3D.x = 120°):
  //   Extended slot (1.75× length) starting −0.75× from centre at both angles
  //   — provides room for layer 0 inward and layer 2 from the tip side.
  //
  // Layer 2 (third-front,  rotation3D.x = 240°):
  //   One full slot at 240° + a short tip slot at 120° (with 180° flip) for
  //   the asymmetric notch that locks the assembly.
  if (numPlanes === 3) {
...
// ─── applySlotCuts ───────────────────────────────────────────────────────────
// Blades from createSlotGeometries are in LOCAL (pre-rotation) space.
// Before sending them to the CSG worker we must rotate each blade by the
// layer's 3D rotation so they align with the already-rotated mesh geometry.
...
const applySlotCuts = async (
  ...
  // Get LOCAL-space slot geometries (not yet rotated into world space)
  const localSlotGeometries = getOrCreateSlotGeometries(
    cacheKey,
  ...
  if (localSlotGeometries.length === 0) return layerGeo;

  // ── Rotate blades into world space to match the already-rotated mesh ──────
  // The mesh was rotated by layer.rotation3D.x/y before applySlotCuts was
  // called. Blades are created in local (flat) space. Apply the same rotation
  // to each blade clone so it cuts in the correct orientation.
  const rxRad = (layer.rotation3D?.x ?? 0) * Math.PI / 180;
  const ryRad = (layer.rotation3D?.y ?? 0) * Math.PI / 180;
  const worldSlotGeometries = localSlotGeometries.map(g => {
    const clone = g.clone();
    clone.rotateX(rxRad);
    clone.rotateY(ryRad);
    return clone;
  ...
  // ── Helper: build result from worker response ─────────────────────────────
  ...
  // ── Fast AABB filter (optional optimisation) ──────────────────────────────
  try {
    ...
    if (layerBB) {
      ...
      for (const g of worldSlotGeometries) {
        try {
          g.computeBoundingBox();
          const gbb = g.boundingBox;
          if (gbb) {
      ...
      if (keptSlots.length === 0) {
        worldSlotGeometries.forEach(s => s.dispose());
        return layerGeo;
      }

      const slotsData = keptSlots.map(g => ({
        positions: Array.from(g.attributes.position.array as Float32Array),
        indices:   g.index ? Array.from(g.index.array as Uint32Array) : null,
        rotation:  { x: 0, y: 0, z: 0 }, // already in world space
      }));

      return postCSGJob(baseData, slotsData, { x: 0, y: 0, z: 0 })
        .then((e: any) => buildResult(e, keptSlots))
        .catch((err: any) => {
          console.error('CSG Worker Error (filtered path):', err);
          worldSlotGeometries.forEach(g => g.dispose());
          return layerGeo;
        });
    }
  } catch (e) {
  ...
  // ── Fallback: send all world-space slots ──────────────────────────────────
  const allSlotsData = worldSlotGeometries.map(g => ({
    positions: Array.from(g.attributes.position.array as Float32Array),
    indices:   g.index ? Array.from(g.index.array as Uint32Array) : null,
    rotation:  { x: 0, y: 0, z: 0 }, // already in world space
  }));

  return postCSGJob(baseData, allSlotsData, { x: 0, y: 0, z: 0 })
    .then((e: any) => buildResult(e, worldSlotGeometries))
    .catch((err: any) => {
...
const App: React.FC = () => {
  ...
  const handleUpdateConfig = useCallback((updates: Partial<SnowflakeConfig>, commitTo3D: boolean = false) => {
    ...
    // Always bust slot caches when anything that affects blade geometry changes,
    // so stale blades from a previous configuration are never reused.
    if ('slotEnabled' in updates || 'slotLength' in updates || 'slotWidth' in updates ||
        'extrusionDepth' in updates || 'bevelEnabled' in updates || 'bevelAmount' in updates ||
        'layers' in updates) {
      clearSlotCache();
      slotCutCache.clear();
    }
                ...
                <div className="w-[420px] flex flex-col border-r border-white/10 bg-slate-900/30 backdrop-blur-sm shrink-0 z-40">
                    <ControlPanel 
                        ...
                        onAutoConfigureSlots={() => {
                          ...
                          const slotMode = activePlanes >= 3 ? '3-plane' : '2-plane';
                          handleUpdateConfig({
                            layers: calculateOptimalSlots(config.layers),
                            slotEnabled: true,
                            slotMode,
                          }, true);
                        }}
