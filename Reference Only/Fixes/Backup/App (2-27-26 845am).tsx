...
const createSlotGeometries = (
  ...
  // ── createBlade ────────────────────────────────────────────────────────────
  // Cuts a slot box from local-X position `xStart` to `xEnd`.
  // Local +X points in the direction the blade will be rotated toward.
  // 0 = hub centre.  Positive = outward along the arm.
  //
  // EPS pads each end so BSP intersection never produces coplanar artifacts.
  const EPS = 2.0;
  const createBlade = (
    xStart:    number,   // local-X start of slot (along blade axis before rotation)
    xEnd:      number,   // local-X end of slot
    thickness: number,   // slot opening width
    extent:    number,   // FULL_PUNCH: punches completely through the mesh thickness
    angleX:    number,   // 90° rotates extent to Z (cuts through extrusion depth)
    angleZ:    number    // rotates blade in XY plane to align with arm direction
  ): THREE_ACTUAL.BufferGeometry => {
    const len    = Math.abs(xEnd - xStart) + EPS * 2;
    const centre = (xStart + xEnd) / 2;
    const geo    = new THREE_ACTUAL.BoxGeometry(len, extent, thickness, 4, 2, 2);
    geo.translate(centre, 0, 0);
    geo.rotateX(angleX * Math.PI / 180);
    geo.rotateZ(angleZ * Math.PI / 180);
    return geo;
  };

  // shorthand for a blade aligned with this layer's arm rotation
  const armBlade = (xStart: number, xEnd: number) =>
    createBlade(xStart, xEnd, cutThickness, FULL_PUNCH, 90, -rotationOffset);

  // ── 2-plane assembly ──────────────────────────────────────────────────────
  // half-back  (layer 0, flat):    slot from centre (0) → outer tip (+drawLength)
  // half-front (layer 1, rot 90°): slot from outer tip (−drawLength) → centre (0)
  // Both use drawLength so they interlock exactly at the hub.
  if (numPlanes === 2) {
    ...
    if (layer.slotType === 'half-back') {
      ...
      slots.push(createBlade(-drawLength, 0, cutThickness, FULL_PUNCH, 90, -angle));
    } else {
      slots.push(createBlade(-drawLength, drawLength, cutThickness, FULL_PUNCH, 90, -angle));
    }
    return slots;
  }

  // ── 3-plane assembly (120° spacing) ───────────────────────────────────────
  // Mirrors the 2D preview (renderSlotPreview in SnowflakePreview.tsx) exactly.
  //
  // Layer 0 — BASE PLANE:
  //   ONE solid slot: centre → outer tip along +rotationOffset°
  //   2D: (0,0) → (cos(angleRad)*drawLength, sin(angleRad)*drawLength)
  //   3D: xStart=0, xEnd=drawLength, angleZ=-angle
  //
  // Layer 1 — CROSS PLANE:
  //   CROSS MAIN solid: centre → outer tip along +angle
  //     2D: (0,0) → (cos(r)*drawLength, …)
  //     3D: xStart=0, xEnd=drawLength, angleZ=-angle
  //   TIP-IN dashed: 75% arm → outer tip along OPPOSITE arm (angle+180°)
  //     2D: xStart=cos(r+π)*adjLength*0.75, xEnd=cos(r+π)*drawLength
  //     blade direction = angle+180°, so local +X is the opposite arm.
  //     3D: xStart=slotLength*0.75, xEnd=drawLength, angleZ=-(angle+180)
  //
  // Layer 2 — TILT PLANE:
  //   TILT MAIN solid: centre → outer tip along +angle
  //     3D: xStart=0, xEnd=drawLength, angleZ=-angle
  //   EXT CHAMFER dashed: centre → 75% arm along OPPOSITE arm (angle+180°)
  //     2D: (0,0) → (cos(r+π)*adjLength*0.75, …)
  //     blade direction = angle+180°, local +X = opposite arm.
  //     3D: xStart=0, xEnd=slotLength*0.75, angleZ=-(angle+180)
  if (numPlanes === 3) {
    const layerIndex = enabledLayers.findIndex(l => l.id === layer.id);
    const angle = rotationOffset;

    if (layerIndex === 0) {
      // BASE PLANE: single slot, centre → outer tip
      slots.push(createBlade(0, drawLength, cutThickness, FULL_PUNCH, 90, -angle));

    } else if (layerIndex === 1) {
      // CROSS PLANE:
      // MAIN: centre → outer tip
      slots.push(createBlade(0, drawLength, cutThickness, FULL_PUNCH, 90, -angle));
      // TIP-IN: 75% of arm → full tip on the OPPOSITE arm direction
      slots.push(createBlade(slotLength * 0.75, drawLength, cutThickness, FULL_PUNCH, 90, -(angle + 180)));

    } else if (layerIndex === 2) {
      // TILT PLANE:
      // MAIN: centre → outer tip
      slots.push(createBlade(0, drawLength, cutThickness, FULL_PUNCH, 90, -angle));
      // EXT CHAMFER: centre → 75% on the OPPOSITE arm direction
      slots.push(createBlade(0, slotLength * 0.75, cutThickness, FULL_PUNCH, 90, -(angle + 180)));
    }

    return slots;
  }

  // ── 4+ planes: full slot through centre ──────────────────────────────────
  slots.push(createBlade(-drawLength, drawLength, cutThickness, FULL_PUNCH, 90, -rotationOffset));
  return slots;
};
...
const App: React.FC = () => {
  ...
  const handleUpdateConfig = useCallback((updates: Partial<SnowflakeConfig>, commitTo3D: boolean = false) => {
    ...
    // Clear slot caches whenever anything that affects blade geometry changes.
    // This prevents stale geometries from a prior session being served from the
    // module-level Map caches.
