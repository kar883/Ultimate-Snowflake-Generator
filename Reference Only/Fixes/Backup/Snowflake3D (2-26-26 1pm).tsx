...
const Snowflake3D: React.FC<Snowflake3DProps> = ({ config, generateMesh, color, undo, redo, canUndo, canRedo, initialDiameter, shortcuts, isVisible }) => {
  ...
  // Sync plane visibility with config layer enabled state
  useEffect(() => {
    setPlaneVisibility(prev => {
      const updated: Record<string, boolean> = {};
      config.layers.forEach((layer, index) => {
        // If plane is enabled in config, turn on visibility toggle
        // If plane is disabled in config, turn off visibility toggle
