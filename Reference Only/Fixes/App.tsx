...
const applySlotCuts = async (
  ...
  console.log('🔧 SLOT CUT DEBUG: applySlotCuts called');
  ...
  try {
    ...
    try {
      ...
      if (layerBB) {
        ...
        console.log('🔧 SLOT CUT DEBUG: About to call postCSGJob - this is where hangs often occur');
...
const App: React.FC = () => {
  ...
  // Ref that is always synchronously up-to-date with rendered3DConfig.
  // Updated directly in setRendered3DIfChanged so generateMesh never reads stale config.
  const rendered3DConfigRef = useRef<SnowflakeConfig>(initialState);
  ...
  const setRendered3DIfChanged = useCallback((next: SnowflakeConfig) => {
    // Update the ref synchronously so generateMesh always reads fresh config
    rendered3DConfigRef.current = next;
  ...
  const handleUpdateConfig = useCallback((updates: Partial<SnowflakeConfig>, commitTo3D: boolean = false) => {
    ...
    // Special handling for color updates - debounce them to prevent spam
    if (updates.color && !updates.slotEnabled && !updates.layers) {
      if (colorDebounceTimer.current) {
        clearTimeout(colorDebounceTimer.current);
      }
      colorDebounceTimer.current = setTimeout(() => {
        console.log('🔍 CONFIG UPDATE DEBUG: Debounced color update applying');
        setConfig(prev => {
  ...
  const generateMesh = useCallback(async (onProgress: (p: number) => void, overrideQuality?: DesignQuality, overrideConfig?: SnowflakeConfig): Promise<THREE_ACTUAL.Group> => {
    console.log('🏗️ MESH GEN DEBUG: Starting mesh generation');
    console.log('🏗️ MESH GEN DEBUG: Config being used:', overrideConfig ? 'override' : 'rendered3DConfig');
    
    const config = overrideConfig || rendered3DConfigRef.current;
    // Always clear geometry caches before generating so stale entries never surface.
    // The caches use content-addressed keys so unchanged geometry will simply be re-created
    // quickly; this avoids the class of bugs where old geometry is served after config changes.
    clearGeometryCache();
    ...
    for (let lIdx = 0; lIdx < layersToGenerate.length; lIdx++) {
      console.log(`🏗️ MESH GEN DEBUG: Processing layer ${lIdx + 1}/${layersToGenerate.length}: ${layersToGenerate[lIdx].name}`);
      ...
      const processTextGroup = async (textGroup: TextGroupConfig) => {
        ...
        try {
          console.log(`🏗️ MESH GEN DEBUG: Loading font "${fontName}" from URL: ${url ? 'custom' : 'built-in'}`);
          let font = await loadFont(fontName, url);
          console.log(`🏗️ MESH GEN DEBUG: Font loaded successfully: ${fontName}`);

          // BOLD FONT VARIANT FALLBACK (E part of B+E)
          // If high stroke weight and bold variant available, try loading it
          const totalStrokeWeight = (config.globalStrokeWeight || 0) + (textGroup.thickness || 0);
          ...
          if (font) {
            ...
            glyphs.forEach((glyph, i) => {
              try {
                ...
                // Advance cursor exactly as the 2D preview does
                currentX += (glyph.advanceWidth || 0) * scale + textGroup.letterSpacing;
              } catch (error) {
            ...
            // Calculate boldness bevel (SVG stroke is centered on path, so expand by strokeWidth/2)
            const boldnessBevel = (shouldApplyBoldness && totalStrokeWeight > 0.1) ? totalStrokeWeight / 2 : 0;
            const needsBevel = config.bevelEnabled || boldnessBevel > 0;

            // Create extrude settings for text (may include boldness bevel on top of regular bevel)
            const textExtrudeSettings = {
              ...extrudeSettings,
              bevelEnabled: needsBevel,
              bevelThickness: bevelPerSide + boldnessBevel,
              bevelSize: bevelPerSide + boldnessBevel,
              bevelSegments: needsBevel ? Math.max(2, bevelSegCap) : 0,
            };

            // (boldness is applied entirely through bevelSize/bevelThickness above)
            ...
            const textKey = makeTextKey(layer.id, textGroup, textGroup.fontSize, effectiveDepth, config.bevelEnabled, bevelPerSide, config.globalStrokeWeight, textGroup.thickness || 0);
            ...
            if (uConf && uConf.enabled) {
                // Match 2D: t = u.thickness + config.globalStrokeWeight (line 152 of SnowflakePreview)
                const t = uConf.thickness + (config.globalStrokeWeight || 0);
                const halfT = t / 2;
            ...
            if (underlineShapes.length > 0) {
                const underlineKey = makeUnderlineKey(layer.id, textGroup, effectiveDepth, config.bevelEnabled, bevelPerSide);
            ...
            for (let i = 0; i < textGroup.arms; i++) {
              ...
              // In 2D: rotate(angle) translate(textX, -mirrorOffset/2) scale(1,-1)
              // After all 3D transforms, armGeo sits at world (textX, -mirrorOffset/2).
              // scale(1,-1,1) flips Y → world (textX, +mirrorOffset/2) which is correct.
              // No additional translate needed.
              if (textGroup.mirrorEnabled) {
            ...
            if (underlineGeo) {
              try {
                // Underline shapes are built in the correct coordinate space (matching the 2D
                // scale(1,-1) context). Do NOT apply rotateX(PI) — that flip is only needed
                // for OpenType glyph paths which are Y-inverted. Underline coords are already correct.
                // When mirrorEnabled, both top+bottom underlines are already baked into underlineShapes,
                // so no additional mirroring is needed in the arm loop.
      ...
      const processHubs = (hubs: HubConfig[]) => {
         ...
         hubs.filter(h => h.enabled).forEach(hub => {
             ...
             const shape = new THREE_ACTUAL.Shape();
             // In 2D the hub is drawn with stroke={globalStrokeWeight} centered on the path edge.
             // This expands the visual outer radius by strokeWeight/2 and shrinks the hollow hole
             // radius by strokeWeight/2 on each side. Match this in 3D.
             const strokeExpand = (config.globalStrokeWeight || 0) / 2;
             const radius = (!isNaN(hub.outerRadius) ? hub.outerRadius : 20) + strokeExpand;
             ...
             if (hub.hollow) {
                 ...
                 for(let i=0; i<=res; i++) {
                     const angle = (i/res) * Math.PI * 2;
                     let r = radius - wallT; // radius already includes strokeExpand; wallT unchanged
                     if (r < 0) r = 0.1;
                     if (hub.shape === 'star') r = (i%2 === 0) ? r : r * sRatio;
                     if (isOsc) r += Math.sin(angle * hub.oscillationFrequency) * amp;
                     const x = Math.cos(angle) * r;
             ...
             const hubKey = makeHubKey(layer.id, hub, effectiveDepth, config.bevelEnabled, bevelPerSide, config.globalStrokeWeight);
      ...
      const processAbstracts = (abstracts: AbstractConfig[]) => {
          ...
          abstracts.filter(a => a.enabled).forEach(abs => {
               // In 2D, abstracts are drawn with strokeWidth = globalStrokeWeight + abs.thickness,
               // centered on the shape edge. This means the rendered half-thickness =
               //   shape_halfT + strokeWidth/2
               //   = abs.thickness/2 + (globalStrokeWeight + abs.thickness)/2
               //   = abs.thickness + globalStrokeWeight/2
               // Match this in 3D by using this as the effective half-thickness.
               const strokeExpand = (config.globalStrokeWeight || 0) / 2;
               const effectiveThickness = abs.thickness * 2 + strokeExpand * 2; // full thickness: both sides
      ...
      if (layer.secondaryEnabled) await processTextGroup(layer.secondary);
    ...
    console.log('🏗️ MESH GEN DEBUG: Mesh generation completed successfully');
    console.log('🏗️ MESH GEN DEBUG: Total layers processed:', layersToGenerate.length);
    console.log('🏗️ MESH GEN DEBUG: Final group children count:', group.children.length);
    onProgress(1);
    return group;
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 relative bg-slate-950 overflow-hidden">
                    ...
                    <div className={`absolute inset-0 w-full h-full transition-opacity duration-300 ${viewMode === '3d' ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                        {console.log('🎨 App Debug - Passing color to Snowflake3D:', config.color)}
                        <Snowflake3D 
