...
async function safeClick(locator: Locator, timeout = 5_000): Promise<void> {
  try {
...
export async function gotoApp(page: Page): Promise<void> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      await expect(page.getByRole('button', { name: /^global$/i }).first()).toBeVisible({ timeout: 30_000 });
      return;
    } catch (error) {
      lastError = error;
...
export async function addAbstractShape(page: Page): Promise<void> {
  const addBtn = page.getByRole('button', { name: /(\+\s*shape|add\s*shape)/i }).first();
  await expect(addBtn).toBeVisible({ timeout: 10_000 });
  await safeClick(addBtn, 5_000);
  await page.waitForTimeout(300);
}
...
export async function addFractal(page: Page): Promise<void> {
  const addBtn = page.getByRole('button', { name: /(\+\s*fractal|add\s*fractal)/i }).first();
  await expect(addBtn).toBeVisible({ timeout: 10_000 });
  await safeClick(addBtn, 5_000);
  await page.waitForTimeout(300);
}
...
export async function commitDeferredTextInput(input: Locator, value: string): Promise<void> {
  ...
  try {
    ...
    // Fallback for deferred inputs that commit on blur.
...
export async function getSvgStats(page: Page): Promise<SvgStats> {
  return page.evaluate((): SvgStats => {
    ...
    if (svgs.length === 0) return { pathCount: 0, bbox: { x: 0, y: 0, width: 0, height: 0 }, fills: [] };

    const scored = svgs.map((s) => {
      const pathCount = s.querySelectorAll('path').length;
      const r = s.getBoundingClientRect();
      const area = Math.max(0, r.width * r.height);
      return { s, pathCount, area };
    });
    scored.sort((a, b) => (b.pathCount - a.pathCount) || (b.area - a.area));
    const svg = scored[0].s;
