...
import { gotoApp, clickTab, waitForSvgStable, clearFocus } from './fixtures';
...
test.describe('Keyboard shortcuts', () => {
  ...
  test('Alt+4 switches to Hubs tab', async ({ page }) => {
    ...
    await page.waitForTimeout(300);
    await expect(page.getByRole('button', { name: /add hub/i })).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('Alt+5 switches to Abstract tab', async ({ page }) => {
    ...
    await page.waitForTimeout(300);
    await expect(page.getByRole('button', { name: /add shape/i })).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('Ctrl+Z undoes a text change', async ({ page }) => {
    ...
    expect(afterUndo).not.toEqual('UNDO_TEST');
  });
  ...
  test('Ctrl+1 toggles between 2D and 3D view', async ({ page }) => {
    await clearFocus(page);
    const canvasBefore = await page.locator('canvas').isVisible();
    await chord(page, '1', ['Control']);
    await page.waitForTimeout(500);
    // The view mode should have toggled (canvas visible/hidden or view mode text changed)
    const canvasAfter = await page.locator('canvas').isVisible();
  ...
  test('Ctrl+S initiates a project save', async ({ page }) => {
    ...
    try {
      ...
      expect(download.suggestedFilename()).toMatch(/\.(json|snowflake)$/i);
    } catch {
      // Some environments suppress the download – just check for no crash
      await expect(page.getByText('Ultimate Snowflake Generator')).toBeVisible();
    }
  });
