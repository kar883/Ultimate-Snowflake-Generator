...
// Efficient structured clone for history
...
const App: React.FC = () => {
  ...
  const setRendered3DIfChanged = useCallback((next: SnowflakeConfig) => {
    setRendered3DConfig(prev => {
      try {
        ...
        // Fallback: if comparison fails, fall through and set
      }
      return next;
    });
  }, []);
  ...
  // Create debounced functions with proper cancellation
  const debouncedUpdate3D = useDebounce(setRendered3DIfChanged, 150); // Reduced from 300ms for better responsiveness
  const debouncedUpdate3DSlow = useDebounce(setRendered3DIfChanged, 500); // Reduced from 1000ms for better UX
  ...
  const generateMesh = useCallback(async (onProgress: (p: number) => void, overrideQuality?: DesignQuality, overrideConfig?: SnowflakeConfig): Promise<THREE_ACTUAL.Group> => {
    ...
    // Clear cache if this is a text property change to ensure regeneration
    if (overrideConfig) {
      const hasTextChanges = overrideConfig.layers.some((l, i) => 
        l.primary.letterSpacing !== rendered3DConfig.layers[i]?.primary.letterSpacing ||
        l.secondary.letterSpacing !== rendered3DConfig.layers[i]?.secondary.letterSpacing ||
        l.primary.textX !== rendered3DConfig.layers[i]?.primary.textX ||
        l.secondary.textX !== rendered3DConfig.layers[i]?.secondary.textX ||
        l.primary.thickness !== rendered3DConfig.layers[i]?.primary.thickness ||
        l.secondary.thickness !== rendered3DConfig.layers[i]?.secondary.thickness ||
        l.primary.rotationOffset !== rendered3DConfig.layers[i]?.primary.rotationOffset ||
        l.secondary.rotationOffset !== rendered3DConfig.layers[i]?.secondary.rotationOffset
      );
      
      if (hasTextChanges) {
        // Only clear affected cache entries, not the entire cache
        const keysToDelete: string[] = [];
        for (const [key] of modelCache3D.entries()) {
          if (key.includes('letterSpacing') || key.includes('textX') || key.includes('thickness') || key.includes('rotationOffset')) {
            keysToDelete.push(key);
          }
        }
        keysToDelete.forEach(key => modelCache3D.delete(key));
      }
    }
    ...
    if (font) {
      ...
      const processImages = (images: ImageConfig[]) => {
        ...
        images.filter(img => img.enabled && img.svgPaths.length > 0).forEach(img => {
          try {
            // Build a minimal SVG string from stored path data
            const svgStr = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${img.svgWidth || 100} ${img.svgHeight || 100}">${img.svgPaths.map(d => `<path d="${d}"/>`).join('')}</svg>`;
            ...
            // Compute bounding box of all raw shapes
            let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
            rawShapes.forEach(shape => {
              shape.getPoints(16).forEach(p => {
            ...
            // Transform: left edge → innerRadius, centre Y → yOffset, flip Y (SVG↓ → THREE↑)
            const s = img.scale;
            const transformPt = (x: number, y: number): [number, number] => [
              (x - minX) * s + img.innerRadius,
              -(y - bboxCenterY) * s + img.yOffset,
            ];
